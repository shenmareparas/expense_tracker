...
│   ├── auth_service.dart     # Authentication layer with Supabase Auth
│   └── database_service.dart # Database operations with Supabase PostgREST and Caching
...
│   └── transaction_viewmodel.dart # Transaction feed, filters, and analytics snapshots
...
│   │   ├── home_page.dart    # Bottom nav layout hosting list & analytics tabs
...
## 💾 Caching Strategy (`DatabaseService`)
...
## 📊 Database Schema & PostgREST Operations

All data columns map strictly between remote database fields and Flutter immutable domain models:
...
| `user_id` | UUID (Foreign Key) | Reference to Supabase Auth User ID |
...
- Ensure all database calls handle exceptions gracefully and return structured models or throw meaningful custom exceptions.
- Leverage `SupabaseConfig.client` to interact with remote tables or auth sessions.
...
3. **Build Services**: Write the backend API interactions first.
4. **Create ViewModels**: Glue the services and properties into observable states. Include loading, success, and error states.
5. **Develop Views & Widgets**: Design stunning responsive interfaces mapped to the ViewModel states.
